const WineDB = {
  sparkling:[
    {name: 'nv Belstar prosecco brut, bisol, veneto, Italy /', nameRus: 'Глера', price: '600 / 3600', id:0, weight: '0,125 / 0,750'},
    {name: 'nv Cava segura viudas brut reserva heredad, catalonia, Spain /', nameRus: 'Макабео Парельяда', price: '6000', id:1, weight: '0,750'},
    {name: 'nv Taittinger brut reserve, champagne, France /', nameRus: 'Пино Нуар, Шардоне, Пино Менье', price: '1250 / 7500', id:2, weight: '0,125 / 0,750'},
    {name: 'nv laurent-perrier brut la cuvee, champagne, France /', nameRus: 'Пино Нуар, Шардоне, Пино Менье', price: '9000', id:3, weight: '0,750'}
  ],
  white:[
    {name: 'varzea do marao, calcada, vinho verde, Portugal /', nameRus: 'Аринту, Лоурейру, Азал, Тражадура', weight: '0,750', price: 3000, id:0},
    {name: 'campogrande orvieto classico, antinori, umbria, Italy /', nameRus: 'Грекето, Проканико', weight: '0,125/0,750', price: '750/3300', id:1},
    {name: '"DR.L" Riesling qualitaswein Semi-sweet .DR. Loosen, Germany /', nameRus: 'Рислинг', weight: '0,125/0,750', price: '650/3900', id:2},
    {name: 'Mud house sauvignon Blanc, marlborough, New Zealand /', nameRus: 'Совиньон Блан', weight: '0,125/750', price: '600/3600', id:3},
    {name: '"Vision" Gruner veltliner organic, markus huber, lovwer Austrial, Austria /', nameRus: 'Грюнер-Вельтлинер', weight: '0,750', price: '4500', id:4},
    {name: 'nv laurent-perrier brut la cuvee, champagne, France /', nameRus: 'Пино Нуар, Шардоне, Пино Менье', weight: '750', price: '9000', id:3},
    {name: 'nv laurent-perrier brut la cuvee, champagne, France /', nameRus: 'Пино Нуар, Шардоне, Пино Менье', weight: '750', price: '9000', id:3},
    {name: 'nv laurent-perrier brut la cuvee, champagne, France /', nameRus: 'Пино Нуар, Шардоне, Пино Менье', weight: '750', price: '9000', id:3},
    {name: 'nv laurent-perrier brut la cuvee, champagne, France /', nameRus: 'Пино Нуар, Шардоне, Пино Менье', weight: '750', price: '9000', id:3},
    {name: 'nv laurent-perrier brut la cuvee, champagne, France /', nameRus: 'Пино Нуар, Шардоне, Пино Менье', weight: '750', price: '9000', id:3},
    {name: 'nv laurent-perrier brut la cuvee, champagne, France /', nameRus: 'Пино Нуар, Шардоне, Пино Менье', weight: '750', price: '9000', id:3},
    {name: 'nv laurent-perrier brut la cuvee, champagne, France /', nameRus: 'Пино Нуар, Шардоне, Пино Менье', weight: '750', price: '9000', id:3},
    {name: 'nv laurent-perrier brut la cuvee, champagne, France /', nameRus: 'Пино Нуар, Шардоне, Пино Менье', weight: '750', price: '9000', id:3},
    {name: 'nv laurent-perrier brut la cuvee, champagne, France /', nameRus: 'Пино Нуар, Шардоне, Пино Менье', weight: '750', price: '9000', id:3},
  ]
}

export default WineDB;